...
export default function ExamBriefingPage({ navigate, paper, subjectSlug = 'mathematics', subjectName = 'Mathematics', variant = 'default' }) {
  ...
  const isEnglishPaperOne = subjectSlug === 'english-a' && isPaperOne;
    ...
    <main className="app-main briefing-main">
      ...
      <section className="briefing-card briefing-card-shell">
        <div className="briefing-copy"><p className="eyebrow">Before you begin</p><h1>{subjectName} · Paper {paper}</h1>
          ...
          {isEnglishPaperTwo ? <div className="briefing-facts english-paper-two-facts"><article><Clock3/><div><strong>{paperVersion ? formatDuration(paperVersion.durationSeconds) : '2 hours 45 minutes'}</strong><span>A visible timer will count down.</span></div></article><article><FileQuestion/><div><strong>Answer 6 questions</strong><span>Questions 1, 2, 3, 6 and 7 are compulsory. Answer either Question 4 or Question 5.</span></div></article><article><Layers3/><div><strong>3 modules · 120 marks</strong><span>Each module includes a 10-mark summary task and a 30-mark extended response.</span></div></article><article><BookOpen/><div><strong>Read, plan, then write</strong><span>Use the source passages, follow each audience and format, and observe the stated word limits.</span></div></article><article><MonitorUp/><div><strong>Stay in this tab</strong><span>Your responses save automatically while you work.</span></div></article></div> : isEnglishPaperOne ? <div className="briefing-facts"><article><Clock3/><div><strong>{paperVersion ? formatDuration(paperVersion.durationSeconds) : '1 hour 30 minutes'}</strong><span>A visible timer will count down.</span></div></article><article><FileQuestion/><div><strong>{paperVersion ? `${paperVersion.questionCount} questions` : '60 questions'}</strong><span>Choose the best answer for every question.</span></div></article><article><BookOpen/><div><strong>Read each passage carefully</strong><span>Use the information and context provided before choosing an answer.</span></div></article><article><PenLine/><div><strong>Complete every item</strong><span>Flag uncertain answers and return to them before submitting.</span></div></article><article><MonitorUp/><div><strong>Stay in this tab</strong><span>Tab switches are noted to protect exam integrity.</span></div></article></div> : <div className="briefing-facts"><article><Clock3/><div><strong>{paperVersion ? formatDuration(paperVersion.durationSeconds) : isPaperOne ? '60 minutes' : '2 hours 40 minutes'}</strong><span>A visible timer will count down.</span></div></article><article><FileQuestion/><div><strong>{paperVersion ? `${paperVersion.questionCount} questions` : isPaperOne ? '60 questions' : '9 questions'}</strong><span>{isPaperOne?'Choose one answer for each question.':'Show your working for method marks.'}</span></div></article><article><PenLine/><div><strong>Get pen and paper ready</strong><span>You may pause here before starting.</span></div></article><article><Calculator/><div><strong>{isPaperOne?'Calculator guidance':'Calculator permitted'}</strong><span>{isPaperOne?'Use one only when the question allows it.':'Keep it nearby for calculations.'}</span></div></article><article><MonitorUp/><div><strong>Stay in this tab</strong><span>Tab switches are noted to protect exam integrity.</span></div></article></div>}
