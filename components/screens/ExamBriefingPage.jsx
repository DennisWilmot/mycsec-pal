    ...
    <main className="app-main briefing-main">
      ...
      <section className="briefing-card briefing-card-shell">
        <div className="briefing-copy"><p className="eyebrow">Before you begin</p><h1>Mathematics · Paper {paper}</h1>
          {loading ? <div className="briefing-load-state"><LoaderCircle className="spin" size={19}/>Checking paper availability…</div> : <>
          ...
          {activeAttempt && <div className="briefing-api-notice"><AlertCircle size={18}/><span>You already have {activeAttempt.subjectName} Paper {activeAttempt.paperNumber} in progress. Resume or cancel it from Practice before starting another paper.</span></div>}
        ...
        <footer className="briefing-card-actions"><span>Make sure you are comfortable and ready before starting.</span><button className="button briefing-start" disabled={loading || starting || !paperVersion || Boolean(activeAttempt)} onClick={startPaper}>{starting ? 'Starting paper…' : `Start Paper ${paper}`} {!starting && <ArrowRight size={18}/>}</button></footer>
      </section>
    </main>
  </div>;
}
