...
export default function RadarChart({axes}){
 const [active,setActive]=useState(null);const cx=190,cy=164,radius=98,n=axes.length;
 ...
 return <svg className="radar interactive-radar" viewBox="0 0 380 340" role="img" aria-label="Interactive performance radar chart">
  ...
  {axes.map((axis,index)=>{const[x,y]=point(index,radius+39);const anchor=Math.abs(x-cx)<18?'middle':x>cx?'start':'end';const labelLines=lines(axis.label);return <text key={axis.label} x={x} y={y-(labelLines.length-1)*7} textAnchor={anchor} className="radar-label">{labelLines.map((line,lineIndex)=><tspan key={line} x={x} dy={lineIndex===0?0:15}>{line}</tspan>)}</text>})}
  {active!==null&&(()=>{const axis=axes[active];const[x,y]=point(active,radius*axis.value/100);const tx=Math.min(292,Math.max(88,x));const ty=Math.max(42,y-42);return <g className="radar-tooltip" pointerEvents="none"><rect x={tx-78} y={ty-27} width="156" height="47" rx="9"/><text x={tx} y={ty-9} textAnchor="middle">{axis.label}</text><text x={tx} y={ty+9} textAnchor="middle">{axis.value}% · {status(axis.value)}</text></g>})()}
 </svg>;
}
