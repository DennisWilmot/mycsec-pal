...
function allowLocally(key:string, limit:number, windowSeconds:number) {
  const now=Date.now(), current=local.get(key);
  if(!current||current.reset<=now){local.set(key,{count:1,reset:now+windowSeconds*1000});return {allowed:true,remaining:limit-1};}
...
export async function allowRequest(key:string, limit:number, windowSeconds:number) {
  if(process.env.RATE_LIMIT_BACKEND==="memory") return allowLocally(key,limit,windowSeconds);
  try {
