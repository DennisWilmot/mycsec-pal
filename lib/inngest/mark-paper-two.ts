...
async function runMarker(parts: Part[]) {
  ...
  const response=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{authorization:`Bearer ${apiKey}`,"content-type":"application/json","x-title":"MyCSECPal Paper 2 Marker"},body:JSON.stringify({model,temperature:0,response_format:{type:"json_schema",json_schema:{name:"paper_two_marks",strict:true,schema:{type:"object",additionalProperties:false,required:["parts"],properties:{parts:{type:"array",items:{type:"object",additionalProperties:false,required:["partId","awardedMarks","feedback","matchedCriteria","missedCriteria"],properties:{partId:{type:"string"},awardedMarks:{type:"number"},feedback:{type:"string"},matchedCriteria:{type:"array",items:{type:"string"}},missedCriteria:{type:"array",items:{type:"string"}}}}}}}}},messages:[{role:"system",content:"You are a careful CSEC Mathematics examiner and an excellent teacher. Award marks only for evidence present in the learner response. Follow the human-reviewed rubric, accept equivalent notation and valid alternative methods, and never exceed the maximum. For each non-blank response, make feedback a concise teaching moment: identify the learner's apparent approach, name what was done correctly, pinpoint the first mathematical or reasoning error, explain why it is wrong, and show the correct next step or method. Refer to the learner's actual numbers, notation, or working so the note could not apply to a different response. If the learner seems to be attempting a recognizable method, say so and explain where it went off track. For a blank response, state the first useful step and concept needed. Do not merely list rubric phrases, narrate what is visible, or give vague advice."},{role:"user",content:JSON.stringify(parts.map((part)=>({partId:part.id,question:part.position,label:part.label,maximumMarks:part.marks,prompt:part.prompt,learnerResponse:part.response,rubric:part.rubric}))) }]} )});
