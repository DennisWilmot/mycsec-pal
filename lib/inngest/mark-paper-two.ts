...
export async function markPaperTwoAttempt(attemptId:string,markingJobId:string){
  ...
  const marks=await runMarker(parts,subjectName), markByPart=new Map(marks.map((mark)=>[mark.partId,mark]));
