...
export function useAttemptSession(attemptId) {
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      if (cancelled) return;
      // Spread clients across a 30-second window and avoid a synchronized
      // request wave at fixed clock intervals.
    ...
    const heartbeat = async () => {
      try {
        const result = await request(`/api/attempts/${attemptId}/heartbeat`, { method: 'POST' });
        const attempt = result?.attempt || result;
        if (attempt?.id) setSession((current) => current ? { ...current, attempt: { ...current.attempt, ...attempt } } : current);
      } catch (caught) {
        if (caught.code === 'ATTEMPT_EXPIRED') {
          setError(caught.message);
          setSession((current) => current ? { ...current, attempt: { ...current.attempt, status: 'submitted', remainingSeconds: 0 } } : current);
        }
      } finally {
        schedule();
      }
