...
export async function POST() { try {
  const { user }=await requireAuthenticatedUser(); const price=process.env.STRIPE_PRICE_PRO;
  ...
  const session=await stripe.checkout.sessions.create({mode:"subscription",customer:customerId,line_items:[{price,quantity:1}],client_reference_id:user.id,subscription_data:{metadata:{profileId:user.id}},success_url:`${applicationUrl()}/settings?billing=success`,cancel_url:`${applicationUrl()}/settings?billing=cancelled`});
